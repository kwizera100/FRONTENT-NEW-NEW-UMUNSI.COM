  ...
  const handleSave = async (publish: boolean) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        router.push(newId ? `/admin/posts/${newId}/edit` : "/admin/posts");
      } else {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-ink-100 p-5">
            ...
            <div className="flex items-center gap-1 mb-3 p-2 bg-ink-50 rounded-xl border border-ink-100">
              ...
              <button onClick={() => insertTag("h3")} className="p-2 rounded-lg hover:bg-white text-ink-600 transition-colors" title="Heading 3">
                <Heading3 className="w-4 h-4" />
              </button>
              <div className="w-px h-5 bg-ink-200 mx-1" />
              <button onClick={() => insertTag("bold")} className="p-2 rounded-lg hover:bg-white text-ink-600 transition-colors" title="Bold">
                <Bold className="w-4 h-4" />
              </button>
              <button onClick={() => insertTag("italic")} className="p-2 rounded-lg hover:bg-white text-ink-600 transition-colors" title="Italic">
                <Italic className="w-4 h-4" />
              </button>
              <div className="w-px h-5 bg-ink-200 mx-1" />
              <button onClick={() => insertTag("quote")} className="p-2 rounded-lg hover:bg-white text-ink-600 transition-colors" title="Quote">
                <Quote className="w-4 h-4" />
              </button>
              <button onClick={() => insertTag("list")} className="p-2 rounded-lg hover:bg-white text-ink-600 transition-colors" title="List">
                <List className="w-4 h-4" />
              </button>
              <div className="w-px h-5 bg-ink-200 mx-1" />
              <button
                onClick={() => { setIsYouTube(false); setMediaStep("choose"); setShowMediaInContent(true); }}
              ...
              <button
                onClick={() => { setShowMediaInContent(true); setMediaStep("youtube"); setIsYouTube(true); }}
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-ink-100 p-5">
            <label className="flex items-center justify-between cursor-pointer">
              ...
              <button
                ...
                className={`relative w-12 h-6 rounded-full transition-colors ${featured ? "bg-brand-600" : "bg-ink-200"}`}
              >
                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${featured ? "translate-x-6" : "translate-x-0.5"}`} />
              </button>
            </label>
            <p className="text-xs text-ink-400 mt-2">
          ...
          <div className="bg-white rounded-2xl border border-ink-100 p-5">
            ...
            <div className="space-y-2">
              {authors.map((name) => (
                <div key={name} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-ink-50 border border-ink-100">
                  ...
                  <button onClick={() => removeAuthor(name)} className="p-1 rounded-lg hover:bg-red-50 text-ink-400 hover:text-red-600 transition-colors">
          ...
          <div className="bg-white rounded-2xl border border-ink-100 p-5">
            ...
            <div className="flex gap-2 mb-3">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg border border-ink-200 focus:border-brand-500 outline-none text-sm"
              />
              <button onClick={addTag} className="px-3 py-2 bg-ink-900 text-white rounded-lg text-sm font-bold">
                <Plus className="w-4 h-4" />
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <span key={tag} className="flex items-center gap-1 px-3 py-1 rounded-lg bg-brand-50 text-brand-700 text-xs font-bold">
                  #{tag}
                  <button onClick={() => setTags(tags.filter((t) => t !== tag))} className="hover:text-brand-900">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
      ...
      {showMediaInContent && (
        <div
          ...
          onClick={() => { setShowMediaInContent(false); setIsYouTube(false); setMediaStep("choose"); }}
        >
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-xl font-black text-ink-900 flex items-center gap-2">
                {isYouTube ? <Youtube className="w-5 h-5 text-red-600" /> : <ImageIcon className="w-5 h-5 text-brand-600" />}
                {isYouTube ? "Add YouTube Video" : "Add Image to Article"}
              </h3>
              <button onClick={() => { setShowMediaInContent(false); setIsYouTube(false); setMediaStep("choose"); }} className="p-2 rounded-lg hover:bg-ink-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {mediaStep === "choose" && (
              <div className="space-y-3">
                <button onClick={() => { setIsYouTube(false); setMediaStep("upload"); }} className="w-full p-4 border-2 border-dashed border-brand-300 hover:border-brand-500 hover:bg-brand-50 rounded-xl text-sm font-bold text-brand-600 flex items-center justify-center gap-3 transition-colors">
                  <ImageIcon className="w-6 h-6" /> Upload from Device
                </button>
                <button onClick={() => { setIsYouTube(false); setMediaStep("url"); }} className="w-full p-4 border-2 border-ink-200 hover:border-ink-400 hover:bg-ink-50 rounded-xl text-sm font-bold text-ink-600 flex items-center justify-center gap-3 transition-colors">
                  <Link2 className="w-6 h-6" /> Paste Image URL
                </button>
                <button onClick={() => { setIsYouTube(false); fetchMediaLibrary(); setMediaStep("library"); }} className="w-full p-4 border-2 border-ink-200 hover:border-ink-400 hover:bg-ink-50 rounded-xl text-sm font-bold text-ink-600 flex items-center justify-center gap-3 transition-colors">
                  <FolderOpen className="w-6 h-6" /> Pick from Media Library
                </button>
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl border border-ink-200 focus:border-brand-500 outline-none text-sm"
                  />
                  <button onClick={() => mediaUrlInput.trim() && setMediaStep("caption")} disabled={!mediaUrlInput.trim()} className="px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-sm disabled:opacity-50 transition-colors">
                    Next
                  </button>
                </div>
                {mediaUrlInput && (
                ...
                <button onClick={() => setMediaStep("choose")} className="text-sm text-ink-400 hover:text-ink-600 font-semibold">&larr; Back</button>
              </div>
            )}
            ...
            {mediaStep === "youtube" && (
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-bold text-ink-700 mb-1.5 block">YouTube URL</label>
                  <input
                    type="text"
                    value={mediaUrlInput}
                    onChange={(e) => setMediaUrlInput(e.target.value)}
                ...
                {mediaUrlInput && getYouTubeId(mediaUrlInput) && (
                  <div className="relative aspect-video rounded-xl overflow-hidden bg-black">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={getYouTubeThumb(mediaUrlInput) || ""} alt="YouTube preview" className="w-full h-full object-cover opacity-80" />
                ...
                {mediaUrlInput && !getYouTubeId(mediaUrlInput) && (
                  <p className="text-xs text-red-500 font-semibold">Invalid YouTube URL. Use a link like https://www.youtube.com/watch?v=...</p>
                )}
            ...
            {mediaStep === "library" && (
              <div className="space-y-4">
                {mediaLibraryLoading ? (
                  ...
                  <>
                    <p className="text-sm text-ink-500 font-semibold">Select an image from your media library:</p>
                    <div className="grid grid-cols-3 gap-3 max-h-80 overflow-y-auto">
                      {mediaLibrary
                        ...
                        .map((media, i) => (
                          <button
                            key={i}
                            onClick={() => { setMediaUrlInput(media.url); setMediaCaptionInput(media.caption || ""); setMediaStep("caption"); }}
            ...
            {mediaStep === "caption" && mediaUrlInput && (
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-sm font-bold text-ink-700 mb-1.5 block">Caption (Optional)</label>
                  <input
                    type="text"
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); insertMediaIntoContent(mediaUrlInput, mediaCaptionInput, isYouTube); } }}
                  />
                  <p className="text-xs text-ink-400 mt-1">
                    {isYouTube ? "Caption will appear below the video. Leave empty to skip." : "Caption will appear below the image. Leave empty to skip."}
                  </p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => setMediaStep(isYouTube ? "youtube" : "choose")} className="px-4 py-2.5 bg-ink-100 hover:bg-ink-200 text-ink-700 font-bold rounded-xl text-sm transition-colors">Back</button>
                  <button onClick={() => insertMediaIntoContent(mediaUrlInput, mediaCaptionInput, isYouTube)} className={`flex-1 py-2.5 text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-colors ${isYouTube ? "bg-red-600 hover:bg-red-700" : "bg-brand-600 hover:bg-brand-700"}`}>
                    <Plus className="w-4 h-4" /> Insert into Article
                  </button>
                </div>
              </div>
            )}
