    ...
    <div className="my-6">
      <div className="bg-gradient-to-br from-[#e5b60d] to-[#c9a00c] rounded-2xl shadow-xl overflow-hidden">
        <div className="p-6 sm:p-8 text-white text-center">
          ...
          <h3 className="text-xl sm:text-2xl font-black mb-2">Iyi nkuru igura {payment.price.toLocaleString()} RWF</h3>
          <p className="text-sm text-white/90 mb-4">
            Wishyure usome iyi nkuru yose ukwezi kose, kandi ugire uburenganzira bwo kuyisoma igihe cyose wifuza.
          </p>
          <div className="bg-white/10 rounded-xl p-3 mb-4 text-left">
            <p className="text-xs font-bold text-white mb-2">Iyo wishyuye ubona:</p>
            <ul className="space-y-1 text-xs text-white/90">
              <li className="flex items-start gap-2"><span className="font-black">1.</span> Verification Badge kuri App yacu ya UMUNSI MEDIA</li>
              <li className="flex items-start gap-2"><span className="font-black">2.</span> Usoma inkuru zihariye ku buntu nta ads zirimo</li>
              <li className="flex items-start gap-2"><span className="font-black">3.</span> Ubasha kuganira n&rsquo;abanyamakuru ba UMUNSI.COM</li>
            </ul>
          </div>
          <button
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-gradient-to-r from-[#e5b60d] to-[#c9a00c] text-white p-4 rounded-t-2xl flex items-center justify-between">
              <h3 className="font-black flex items-center gap-2"><Crown className="w-5 h-5" /> Wishyure usome</h3>
              <button onClick={() => setShowForm(false)} className="p-1 rounded-full hover:bg-white/20">×</button>
            </div>
            <div className="p-4">
              {!selectedMethod ? (
                <>
                  <p className="text-sm text-gray-600 mb-4 text-center">Hitamo nzira y&rsquo;ishyura:</p>
                  <div className="space-y-2">
                    {config?.paymentMethods.mtn.enabled && (
                      <button onClick={() => setSelectedMethod("mtn")} className="w-full text-left p-3 rounded-xl border-2 border-gray-200 hover:border-[#e5b60d]/50 flex items-center gap-3">
                        ...
                        <div className="flex-1"><p className="font-bold text-sm">{config.paymentMethods.mtn.label}</p>{config.paymentMethods.mtn.comingSoon && <span className="text-xs text-orange-500">Baza vuba</span>}</div>
                      </button>
                    )}
                    {config?.paymentMethods.airtel.enabled && (
                      <button onClick={() => setSelectedMethod("airtel")} className="w-full text-left p-3 rounded-xl border-2 border-gray-200 hover:border-[#e5b60d]/50 flex items-center gap-3">
                        ...
                        <div className="flex-1"><p className="font-bold text-sm">{config.paymentMethods.airtel.label}</p>{config.paymentMethods.airtel.comingSoon && <span className="text-xs text-orange-500">Baza vuba</span>}</div>
                      </button>
                    )}
                    {config?.paymentMethods.bank.enabled && (
                      <button onClick={() => setSelectedMethod("bank")} className="w-full text-left p-3 rounded-xl border-2 border-gray-200 hover:border-[#e5b60d]/50 flex items-center gap-3">
                        ...
                        <div className="flex-1"><p className="font-bold text-sm">{config.paymentMethods.bank.label}</p><span className="text-xs text-green-600">Biri ku murongo!</span></div>
                      </button>
                    )}
                  </div>
                </>
                ...
                <>
                  <button onClick={() => setSelectedMethod("")} className="text-xs text-gray-500 mb-3">&larr; Subira inyuma</button>
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-semibold text-gray-600 mb-1">Amazina *</label>
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} maxLength={100} placeholder="Amazina yawe" className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-200 focus:border-[#e5b60d] outline-none text-sm" />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} maxLength={200} placeholder="email@urugendo.rw" className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-200 focus:border-[#e5b60d] outline-none text-sm" />
                      </div>
                    </div>
                    <div>
                  ...
                  <button onClick={handleSubmit} disabled={loading} className="w-full mt-4 inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#e5b60d] hover:bg-[#c9a00c] disabled:opacity-60 text-white font-bold transition-colors text-sm">
                    {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Kohereza...</> : `Emeza ${payment.price.toLocaleString()} RWF`}
                  </button>
                </>
              )}
            </div>
      ...
      {comingSoon && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => { setComingSoon(false); setShowForm(false); }}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 text-center">
            ...
            <h3 className="font-bold text-gray-900 mb-2">Iyi nzira baza vuba cyane!</h3>
            <p className="text-sm text-gray-500 mb-4">MTN na Airtel bizaba ku murongo vuba. Kuri none, hitamo Bank Account.</p>
            <button onClick={() => { setComingSoon(false); setSelectedMethod(""); }} className="px-6 py-2.5 rounded-xl bg-[#e5b60d] hover:bg-[#c9a00c] text-white font-bold text-sm">Hitamo izindi nzira</button>
          </div>
        </div>
      )}
      ...
      {success && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 text-center">
            ...
            <h3 className="font-bold text-gray-900 mb-2">Murakoze! Wishyuye neza.</h3>
            ...
            {bankInfo && (
              <div className="bg-gray-50 rounded-xl p-3 text-left mb-3 text-xs">
                ...
                <p><strong>Konti:</strong> {bankInfo.accountName}</p>
                <p><strong>Numero:</strong> {bankInfo.accountNumber}</p>
                <p><strong>Amfaranga:</strong> {bankInfo.amount?.toLocaleString()} {bankInfo.currency}</p>
              </div>
            )}
            <button onClick={() => { setSuccess(false); setShowForm(false); setHasAccess(true); }} className="px-6 py-2.5 rounded-xl bg-[#e5b60d] hover:bg-[#c9a00c] text-white font-bold text-sm">Soma inkuru</button>
          </div>
        </div>
      )}
    </div>
