...
export default function AdminCommentsPage() {
  ...
  const deleteComment = async (id: string) => {
    if (!confirm("Niba ukomeza igitekerezo kizasibwa. Urabyemeza?")) return;
    setActionLoading(id);
    try {
  ...
  const submitReply = async (id: string) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (data.success) {
        setMessage("Igisubizo cyoherejwe kuri email ya user!");
        setReplyingTo(null);
        setReplyText("");
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">Bitegereje</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
          <p className="text-2xl font-black text-green-600">{approved.length}</p>
          <p className="text-xs text-gray-500 mt-1">Byemejwe</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-400 mt-3">Biripakira...</p>
        </div>
      ) : comments.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-xl border border-gray-100">
          <MessageCircle className="w-12 h-12 text-gray-300 mx-auto mb-3" />
          <p className="text-gray-400">Nta bitekerezo byabonetse.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <div className="flex flex-col items-end gap-1">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${c.isSpam ? "bg-red-100 text-red-700" : c.isApproved ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                    {c.isSpam ? "Spam" : c.isApproved ? "Byemejwe" : "Bitegereje"}
                  </span>
                  <span className="text-xs text-gray-400">
                    {new Date(c.createdAt).toLocaleDateString("rw-RW", { year: "numeric", month: "short", day: "numeric" })}
                  </span>
                </div>
              </div>
              ...
              {replyingTo === c.id && (
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <textarea
                    ...
                    rows={3}
                    placeholder="Andika igisubizo cyawe... Kizoherejwa kuri email ya user ako kanya."
